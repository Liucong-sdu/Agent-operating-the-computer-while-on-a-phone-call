...
class TaskExecutor:
    ...
    def __init__(self, test_url: str = "https://www.baidu.com"):
        ...
        logger.info(f"任务执行器初始化完成，将使用Browser-Use统一管理浏览器访问: {test_url}")
    ...
    async def _initialize_browser_agent(self) -> BrowserAgent:
        ...
        try:
            ...
            if not api_key:
                raise ValueError("未找到ANTHROPIC_API_KEY环境变量，请检查.env文件配置")
            
            # 创建Browser-Use代理
            agent = BrowserAgent(
                api_key=api_key,
                model="claude-sonnet-4-20250514",  # 使用Claude模型
                temperature=1.0
            )
    ...
    def _build_full_task_description(self, task_description: str, target_url: str) -> str:
        ...
        if any(keyword in task_lower for keyword in ["打开", "访问", "导航到", "跳转到", "进入"]):
            # 如果已经包含导航指令，确保URL明确
            if target_url not in task_description:
                return f"{task_description}。具体网址是: {target_url}"
            return task_description
        else:
            # 如果没有，添加明确的打开网址指令
            return f"请直接导航到网址: {target_url}，然后{task_description}。重要：必须先成功打开 {target_url} 这个网址。"
